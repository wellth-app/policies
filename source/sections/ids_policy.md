...
In order to preserve the integrity of data that Wellth stores, processes, or transmits for Customers, Wellth implements strong intrusion detection tools and policies to proactively track and retroactively investigate unauthorized access. Wellth currently utilizes [OSSEC](http://www.ossec.net/) to track file system integrity, monitor log data, and detect rootkit access.
...
* Wellth firewalls monitor all incoming traffic to detect potential denial of service attacks. Suspected attack sources are blocked automatically. Additionally, our hosting provider actively monitors its network to detect denial of services attacks.
...
* Wellth utilizes redundant firewall on network perimeters.
* Static IP addresses are used for Wellth servers.
